...
## 🗑️ v1.2.7 - 删除文件工具 + 工具调用限制强化版 (2025-08-20)

### 🆕 新增功能
- ✅ **删除文件工具** - AI可以安全删除不需要的文件
  - 支持格式：`<delete_file><path>文件路径</path></delete_file>`
  - 完整的安全检查：文件存在性、类型验证
  - 详细预览显示：文件大小、内容预览（前3行）
  - 权限控制集成：Ask模式禁止，mostly accepted需确认，sprint自动执行

### 🚨 工具调用限制强化
- 🔧 **单工具调用限制** - 每次AI响应只能调用一个工具
  - 检测多个工具调用，只执行第一个
  - 其他工具被忽略，提示AI下次单独调用
  - 防止工具调用混乱和冲突

- 🛠️ **错误处理强化** - 工具执行失败时必须继续
  - 所有工具执行包装在try-catch中
  - 生成详细错误信息和修复建议
  - 绝不因工具失败而结束任务
...
- 📈 **对话限制提升** - 最大迭代次数从15次提升到50次
  - 给AI更多机会完成复杂任务
  - 重复限制保持3次不变

### 📝 提示词全面强化
- ✅ **所有4种强度** - Claude、Flash、Qwen、Mini都添加工具调用限制说明
- ✅ **删除文件工具** - 在所有提示词中添加删除工具说明和安全注意事项
- ✅ **双重保护机制** - 软件层面强制执行 + AI层面明确告知

### 🎯 解决的核心问题
- ❌ **AI遇到错误直接结束** → ✅ **AI分析错误并持续修复**
- ❌ **同时调用多个工具** → ✅ **每次只调用一个工具**
- ❌ **工具失败就放弃** → ✅ **工具失败时继续尝试**
- ❌ **对话次数限制太低** → ✅ **提升到50次给更多机会**
- ❌ **缺少删除文件功能** → ✅ **安全的删除文件工具**
